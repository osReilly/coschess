...
cc.Class({
    ...
    joinroom_event:function(data , context){
        ...
        let players = data.playUser
       // this.player =  players
        let index = players.length -1
        console.log(index,this.game)
        debugger
        if(index === 0){
            cc.beimi.user.index = 0
        }else if(index === 1){
            console.log('----players[1]-----',players[1])
            if(cc.beimi.user.index === 0){
                context.newplayer(players.length , context , players[1] , true) ;          
            }else{
                cc.beimi.user.index = 1
                context.newplayer(players.length , context , players[0], false) ;          
            }
        }else{
             if(cc.beimi.user.index === 0){
                // context.newplayer(players.length , context , players[1] , true) ;  
                context.newplayer(players.length , context , players[2], false) ;          
            }else if(cc.beimi.user.index === 1){
                context.newplayer(players.length , context , players[2], true) ;          
            }else{
                  cc.beimi.user.index = 2
                context.newplayer(players.length , context , players[0] , true) ;  
                context.newplayer(players.length , context , players[1], false) ;      
            }
        }
        if(players.length ===3){
       ...
       //          console.log('cc.beimi.user.id === player.id--',cc.beimi.user.id , player.id)
       //      if(cc.beimi.user.id !== player.id){
       //          context.newplayer(players.length , context , player , players.length == 0 && !(players.length < cc.beimi.user.players)) ;
               
       //      }else{
       //         //  cc.beimi.user.roomIndex = player.roomIndex
       //         // var render =  this.game.playermysql.getComponent("PlayerRender") ;
       //         //  render.username.string = cc.beimi.user.nickname + cc.beimi.user.roomIndex;
       ...
       //  if(data.playUser &&  data.playUser[0].id == cc.beimi.user.id){
       //      //本人，开始计时
       //      //console.log("本人") ;
       //      //self.player[0] = data ;
    ...
    breakroom_event(data,context){
        console.log(context.player)
    debugger
        var render = context.player.getComponent("PlayerRender") ;
    ...
    onDestroy:function(){
        ...
        if(this.ready()){
            let socket = this.socket();
            socket.emit("break_room",{user:cc.beimi.user});
        }
    }
});
